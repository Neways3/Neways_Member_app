...
import 'package:super_home_member/screens/gaming_zone/ludo/players/players.dart';
import 'package:super_home_member/screens/gaming_zone/ludo/players/players_notifier.dart';
...
import 'collision_details.dart';

class FludoGame2 extends StatefulWidget {
  @override
  _FludoGameState createState() => _FludoGameState();
}

class _FludoGameState extends State<FludoGame2> with TickerProviderStateMixin {
  ...
  late int _stepCounter = 0,_diceOutput = 0,_currentTurn = 0,_selectedPawnIndex,
      _maxTrackIndex = 57,_straightSixesCounter = 0,_forwardStepAnimTimeInMillis = 250,
                          ...
                          SizedBox.expand(
                            child: CustomPaint(
                              painter: BoardPainter(
                                  trackCalculationListener: (playerTracks) async{
  ...
  List<Widget> _buildPawnWidgets() {
    ...
    for (int playerIndex = 0; playerIndex < 4; playerIndex=playerIndex+2) {
      Color playerColor;
      switch (playerIndex) {
        case 0:
      ...
      playerIndex++) {
        ...
        pawnIndex++) {
          if ((playerIndex != _currentTurn ||
              pawnIndex != _selectedPawnIndex) && _pawnCurrentStepInfo[
  ...
  _changeTurn() {
    if (_winnerPawnList.where((playerPawns) {
      ...
      if (!_provideFreeTurn) {
        do {
          //to ignore winners
          _currentTurn =
              (_currentTurn + 2) % 4; //change turn after animation completes
          if (_winnerPawnList[_currentTurn].length != 4) {
    ...
    if(_currentTurn == AppColors.player1){
...
import 'package:flutter/services.dart';
import 'package:provider/provider.dart';
import 'package:super_home_member/screens/gaming_zone/ludo/players/diceVisibilityController.dart';
import 'package:super_home_member/screens/gaming_zone/ludo/players/players.dart';
import 'package:super_home_member/screens/gaming_zone/ludo/players/players_notifier.dart';
import '../board/board.dart';
import '../board/overlay_surface.dart';
...
class _FludoGameState extends State<FludoGame2> with TickerProviderStateMixin implements DiceVChange{
  late Animation<Color?> _playerHighlightAnim;
  late Animation<double> _diceHighlightAnim;
  late AnimationController _playerHighlightAnimCont, _diceHighlightAnimCont;
  List<List<AnimationController>> _playerAnimContList = [];
  ...
  void initState() {
    ...
    Get.lazyPut(() => DiceVisibilityController(), fenix: true);
          ...
          children: <Widget>[
            ...
            Center(
              child: Column(
                mainAxisSize: MainAxisSize.min,
                children: <Widget>[
                  //if(shouldDiceVisibleForThisPerson() && _currentTurn)
                  Consumer<DiceNotifier>(builder: (_, notifier, __) {
                    if (notifier.isRolled) {
                      _highlightCurrentPlayer();
                      _diceOutput = notifier.output;
                    ...
                    return Visibility(
                        visible: notifier.visibilityFor==2,
                        //true,
                        //Get.find<DiceVisibilityController>().visibilityFor.value == 0,
                  ...
                  Consumer<DiceNotifier>(builder: (_, notifier, __) {
                    ...
                    return Visibility(
                        visible:
                        //true,//
                        notifier.visibilityFor==0,
                        //Get.find<DiceVisibilityController>().visibilityFor.value == 2,
                        child: GestureDetector(
                          onTap: () {
                            //print("1Roll Dice Called ${notifier.visibilityFor}");
                            if (_diceHighlightAnimCont.isAnimating) {
                              _playerHighlightAnimCont.reset();
                              _diceHighlightAnimCont.reset();
                              _diceNotifier.rollDice();
  ...
  bool shouldDiceVisibleForThisPerson(){
    var isValid = false;
...
//await Future.delayed(Duration(milliseconds: 100));
  ...
  onTurnChange() async {
    ...
    if(_currentTurn==0){
    _diceNotifier.visibilityFor = 0;
    }else{
    _diceNotifier.visibilityFor = 2;
    }
    print("onTurnChange: _currentTurn: $_currentTurn, visibilityFor: ${_diceNotifier.visibilityFor}");
    //await Future.delayed(Duration(milliseconds: 100));
